...
if(isset($_REQUEST['id'])){
  ...
  mysqli_query($conn,"DELETE FROM approval_table WHERE id=".$_REQUEST['id']);
		echo "<script>location.href='approvaled.php?pg=approve';</script>";
	}

}
if(isset($_REQUEST['block'])){
	$res =mysqli_query($conn,"SELECT photo,email FROM approval_table WHERE id=".$_REQUEST['block']);
	$rw =mysqli_fetch_array($res);
	$photo=$rw["photo"];
	$email=$rw["email"];

	$del=mysqli_query($conn,"UPDATE users SET status='pending' WHERE email='$email'");
	if($del){
  mysqli_query($conn,"UPDATE approval_table SET status='pending' WHERE id=".$_REQUEST['block']);
		echo "<script>location.href='approved.php?pg=approve';</script>";
	}
...
<table id="myTable" border="1px" >
  ...
  <?php
	...
	$query0 = mysqli_query($conn,"SELECT * FROM approval_table WHERE  status='Approved'");
  $i=1;
...
?>
  <tr>
    <td><?php echo $i ?></td>
		...
		<td>
			 <button style="background-color: green; width:100px;" > <a style="color:white; text-decoration:none;" href="?pg=approve&block=<?php echo $row0["id"];?>">Block</a></button><br>
		 <?php if($_SESSION["LEVEL"]=="Admin"){

		   ?>
		<button style="background-color: red; width:100px;" ><a style="color:white; text-decoration:none;" href="?pg=approve&id=<?php echo $row0["id"];?>">Delete</a></button>
<?php } ?>
		</td>
  </tr>
<?php
